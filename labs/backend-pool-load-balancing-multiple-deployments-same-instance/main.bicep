...
var tmp = [
  for config in indexedOpenAIConfig: map(
    config.models,
    (m, i) => {
      name: config.name
      location: config.location
...
//     for deploymentName in flattenedOpenAIConfig: {
//       deploymentName: deploymentName
//       backends: filter(fla[for (config, i) in flattenedOpenAIConfig: (config.deploymentName == deploymentName) ? config : []]
//     }
//   ]

// output twoDimensionalArray array = twoDimensionalArray


// var backendsByDeploymentName = [
//     for depName in distinctDeploymentNames: {
//         deploymentName: [depName]
//         //backends: [for (config, i) in flattenedOpenAIConfig: (config.deploymentName == depName) ? config : []]
...
// func assignBackendPoolServices(allBackends array, depName string) array => {

//     var backendsForPoolTmp = [for (item, i) in allBackends: item.deployment == depName ? item : []]
//     var backendsForPool = intersection(backendsForPoolTmp, allBackends)     // eliminates empty arrays

// }


//var backendsForSpecificDeployment = filter(backendsByDeploymentName, (config) => config.deploymentName == 'gpt-35-turbo-1106')[0].backends
...
resource backendPoolOpenAI 'Microsoft.ApiManagement/service/backends@2023-09-01-preview' = [
    for (deploymentName, i) in distinctDeploymentNames: if (length(distinctDeploymentNames) > 0) {
      ...
      properties: {
        ...
        type: 'Pool'
        pool: {
        //   services: [for (config, i) in flattenedOpenAIConfig: (config.deploymentName == deploymentName) ? {
        //       id: '/backends/${config.name}-${config.deploymentName}'
        //       priority: config.priority
        //       weight: config.weight
        //     } : {}
        //   ]
        //   services: [for (config, i) in map(flattenedOpenAIConfig, (c) => (c.deploymentName == deploymentName)): {
        //     //   id: '/backends/${config.name}-${config.deploymentName}'
        //     //   priority: config.priority
        //     //   weight: config.weight
        //     weight: config
        //     }
        //   ]
            //services: assignBackendPoolServices(flattenedOpenAIConfig, deploymentName)
            services: [for (config, i) in filter(backendsByDeploymentName, (config) => config.deploymentName == deploymentName)[0].backends: {
                id: '/backends/${config.name}-${config.deploymentName}'
                priority: config.priority
                weight: config.weight
            }]
...
// someProperty: [for (config) in myArray: (config.X == X) ? {
//     id: config.Id
//   } : {}
// ]
