...
resource cognitiveServices_1 'Microsoft.CognitiveServices/accounts@2023-05-01' = [for config in openAIConfig_1: if(length(openAIConfig_1) > 0) {
...
resource cognitiveServices_2 'Microsoft.CognitiveServices/accounts@2023-05-01' = [for config in openAIConfig_2: if(length(openAIConfig_2) > 0) {
  name: '${config.name}-${resourceSuffix}'
  location: config.location
  sku: {
    name: openAISku
...
resource cognitiveServices_3 'Microsoft.CognitiveServices/accounts@2023-05-01' = [for config in openAIConfig_3: if(length(openAIConfig_3) > 0) {
  name: '${config.name}-${resourceSuffix}'
  location: config.location
  sku: {
    name: openAISku
...
// Named Value to add Azure OpenAI deployment to backend mapping
resource namedValue 'Microsoft.ApiManagement/service/namedValues@2023-09-01-preview' = {
  name: 'Deployment-to-Backend-Mapping'
  parent: apimService
  properties: {
    displayName: 'Deployment-to-Backend-Mapping'
    secret: false
    value: '{\\"gpt-35-turbo-0125\\":\\"oai-backend-pool-gpt-35-turbo-0125\\",\\"gpt-35-turbo-1106\\":\\"oai-backend-pool-gpt-35-turbo-1106\\",\\"gpt-4o-2024-05-13\\":\\"oai-backend-pool-gpt-4o-2024-05-13\\"}'
  }
}

// Add APIM's system-assigned managed identity to each Azure OpenAI resource and assign the appropriate role
...
resource roleAssignment_1 'Microsoft.Authorization/roleAssignments2@2022-04-01' = [for (config, i) in openAIConfig_1: if(length(openAIConfig_1) > 0) {
...
resource apiDiagnostics 'Microsoft.ApiManagement/service/apis/diagnostics@2023-09-01-preview' = if (!empty(apimLogger.name)) {
